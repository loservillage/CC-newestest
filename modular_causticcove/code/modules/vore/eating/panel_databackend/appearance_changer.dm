		...
		if("rename")
			if(owner)
				...
				if(!isnull(raw_name) && can_change(owner, APPEARANCE_RACE))
					var/new_name = sanitize_name(raw_name) // can't edit synths
					if(new_name)
...
/datum/tgui_module/appearance_changer/ui_interact(mob/user, datum/tgui/ui = null)
	if(customize_usr && !owner)
		if(!ishuman(user))
			return TRUE
		owner = user

	if(!owner || !owner.client || !owner.client.prefs.pref_species)
		return

	ui = SStgui.try_update_ui(user, src, ui)
	if(!owner || !owner.client || !owner.client.prefs.pref_species) //We tried to update our UI and no longer have an owner!
		return
	if(!ui)
...
/*/datum/tgui_module/appearance_changer/proc/update_dna(mob/living/carbon/human/target) //This MIGHT be solely for Genetics DNA in regular SS13
	if(target)
		target.update_dna()*/
...
/datum/tgui_module/appearance_changer/proc/get_genders(mob/living/carbon/human/target)
	var/datum/species/S = target.mind.species
