...
/obj/structure/stone_tile/Crossed(atom/movable/AM)
	...
	switch(fall_on_cross)
		if(COLLAPSE_ON_CROSS, DESTROY_ON_CROSS)
			if((I && I.w_class >= WEIGHT_CLASS_BULKY) || (L && !(L.movement_type & FLYING) && L.mob_size >= MOB_MEDIUM)) //too heavy! too big! aaah! //Caustic Edit - Changed MOB_SIZE_HUMAN to MOB_MEDIUM
				collapse()
		if(UNIQUE_EFFECT)
			crossed_effect(AM)
